...
async function seedMPASIRecipes() {
    ...
    try {
        ...
        await prisma.mPASIRecipe.deleteMany()
        console.log('✅ Cleared existing MPASI recipes')

        // Insert sample recipes
        for (const recipe of sampleRecipes) {
            await prisma.mPASIRecipe.create({
                data: recipe
            })
            console.log(`✅ Created recipe: ${recipe.name}`)
        }

        console.log(`🎉 Successfully seeded ${sampleRecipes.length} MPASI recipes!`)
    } catch (error) {
        console.error('❌ Error seeding MPASI recipes:', error)
        throw error
    } finally {
